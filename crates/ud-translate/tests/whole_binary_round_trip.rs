//! Whole-binary source round-trip property:
//!
//! ```text
//! lower_to_elf(parse(decompile_to_text(elf)))   ==   original ELF bytes
//! ```
//!
//! The stronger property than per-section: the entire ELF — ehdr,
//! program-header table, section-header table, every section's
//! bytes, and inter-region padding — round-trips through `.ud`
...
#[test]
fn whole_binary_byte_identity_through_source() {
    ...
    assert!(
        failures.is_empty(),
        "{} fixture(s) failed whole-binary round-trip:\n  {}",
        failures.len(),
        failures.join("\n  ")
    );
    eprintln!("whole-binary round-trip: {total} fixtures, {total_bytes} bytes");
    assert!(total > 0, "expected at least one fixture to test");
}
