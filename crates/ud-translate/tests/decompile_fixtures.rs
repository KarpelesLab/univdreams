...
/// either direction.
///
/// Functions flagged `#[autogen_pro]` / `#[autogen_epi]` have their
/// prologue / epilogue dropped from the body — the bytes get
/// regenerated at lower time. To keep this invariant honest, route
/// every function's body through `lower_function_bytes` (which
/// re-injects the regenerated bytes when the autogen marker is set)
/// before counting.
#[test]
fn asm_count_matches_lifted_instruction_count() {
    ...
    let asm_count = count_function_bytes_as_insns(&ast.items);
        ...
        for item in items {
            match item {
                Item::Function(f) => {
                    ...
                    let bytes = ud_translate::compile::lower_function_bytes_at(
                        f,
                        ip,
