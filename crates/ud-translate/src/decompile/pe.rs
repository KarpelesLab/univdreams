...
fn build_pe_module(pe: &PeFile) -> Module {
    ...
    let dos_block = build_dos_block(&pe.dos);
    let dos_stub_value = byte_list(&pe.dos_stub);
    let optional_value = pe
        .optional
        .as_ref()
        .map_or(Value::Block(Vec::new()), build_optional_block);
    let data_directories_value = Value::List(
        pe.data_directories
            .iter()
            .map(|dd| {
                Value::Block(vec![
                    field("virtual_address", Value::Int(u64::from(dd.virtual_address))),
                    field("size", Value::Int(u64::from(dd.size))),
                ])
            })
            .collect(),
    );
    ...
    let build = Value::Block(vec![
        ...
        field("dos", dos_block),
        field("dos_stub", dos_stub_value),
        field("coff", coff_block),
        field("optional", optional_value),
        field("data_directories", data_directories_value),
...
fn build_pe_items(pe: &PeFile) -> Vec<Item> {
    ...
    // The DOS header, DOS stub, COFF header, optional header,
    // data directories, and section table are all captured
    // structurally in `@module.build` and regenerated by the
    // PE lower path. Compute their end offset so the leading
    // @raw block starts past them — only the alignment padding
    // between the section table end and the first section's
    // raw data needs to ride along as bytes.
    let header_end = pe_header_region_end(pe);

    let mut cursor = header_end.min(bytes.len());
    for (start, end, sec_idx) in &regions {
        if *start > cursor {
            // Trailing alignment between section table and
            // first section's raw data. Small, often a few
            // hundred bytes for file-alignment.
            items.push(Item::Raw {
                addr: cursor as u64,
...
/// File offset at which the PE skeleton (DOS header + DOS stub +
/// PE signature + COFF header + optional header + data
/// directories + section table) ends. Bytes from offset 0 up to
/// this point are fully covered by `@module.build` structured
/// fields, so the decompile path no longer emits an `@raw` for
/// them — the lower path reconstructs the bytes from the
/// structured fields via `PeFile::from_parts`.
fn pe_header_region_end(pe: &PeFile) -> usize {
    const PE_SIG_SIZE: usize = 4;
    const COFF_HEADER_SIZE: usize = 20;
    let opt_size = pe.coff.size_of_optional_header as usize;
...
fn build_dos_block(dos: &ud_format::pe::DosHeader) -> Value {
    Value::Block(vec![
        ...
        field("e_cp", Value::Int(u64::from(dos.e_cp))),
        field("e_crlc", Value::Int(u64::from(dos.e_crlc))),
        field("e_cparhdr", Value::Int(u64::from(dos.e_cparhdr))),
        field("e_minalloc", Value::Int(u64::from(dos.e_minalloc))),
        field("e_maxalloc", Value::Int(u64::from(dos.e_maxalloc))),
        field("e_ss", Value::Int(u64::from(dos.e_ss))),
        field("e_sp", Value::Int(u64::from(dos.e_sp))),
        field("e_csum", Value::Int(u64::from(dos.e_csum))),
        field("e_ip", Value::Int(u64::from(dos.e_ip))),
        field("e_cs", Value::Int(u64::from(dos.e_cs))),
        field("e_lfarlc", Value::Int(u64::from(dos.e_lfarlc))),
        field("e_ovno", Value::Int(u64::from(dos.e_ovno))),
        field(
            "e_res",
            Value::List(
                dos.e_res
                    .iter()
                    .map(|n| Value::Int(u64::from(*n)))
                    .collect(),
            ),
        ),
        field("e_oemid", Value::Int(u64::from(dos.e_oemid))),
        field("e_oeminfo", Value::Int(u64::from(dos.e_oeminfo))),
        field(
            "e_res2",
            Value::List(
                dos.e_res2
        ...
        field("e_lfanew", Value::Int(u64::from(dos.e_lfanew))),
    ])
}
...
fn build_optional_block(opt: &ud_format::pe::OptionalHeader) -> Value {
    Value::Block(vec![
        ...
        field(
            ...
            Value::Int(u64::from(opt.major_linker_version)),
        ),
        field(
            "minor_linker_version",
            Value::Int(u64::from(opt.minor_linker_version)),
        ),
        field("size_of_code", Value::Int(u64::from(opt.size_of_code))),
        field(
            "size_of_initialized_data",
            Value::Int(u64::from(opt.size_of_initialized_data)),
        ),
        field(
            "size_of_uninitialized_data",
            Value::Int(u64::from(opt.size_of_uninitialized_data)),
        ),
        field(
            "address_of_entry_point",
            Value::Int(u64::from(opt.address_of_entry_point)),
        ),
        field("base_of_code", Value::Int(u64::from(opt.base_of_code))),
        field("base_of_data", Value::Int(u64::from(opt.base_of_data))),
        field("image_base", Value::Int(opt.image_base)),
        field(
            "section_alignment",
            Value::Int(u64::from(opt.section_alignment)),
        ),
        field("file_alignment", Value::Int(u64::from(opt.file_alignment))),
        field(
            "major_operating_system_version",
            Value::Int(u64::from(opt.major_operating_system_version)),
        ),
        field(
            "minor_operating_system_version",
            Value::Int(u64::from(opt.minor_operating_system_version)),
        ),
        field(
            "major_image_version",
            Value::Int(u64::from(opt.major_image_version)),
        ),
        field(
            "minor_image_version",
            Value::Int(u64::from(opt.minor_image_version)),
        ),
        field(
            "major_subsystem_version",
            Value::Int(u64::from(opt.major_subsystem_version)),
        ),
        field(
            "minor_subsystem_version",
            Value::Int(u64::from(opt.minor_subsystem_version)),
        ),
        field(
            "win32_version_value",
            Value::Int(u64::from(opt.win32_version_value)),
        ),
        field("size_of_image", Value::Int(u64::from(opt.size_of_image))),
        field(
            "size_of_headers",
            Value::Int(u64::from(opt.size_of_headers)),
        ),
        field("check_sum", Value::Int(u64::from(opt.check_sum))),
        field("subsystem", Value::Int(u64::from(opt.subsystem))),
        field(
            "dll_characteristics",
            Value::Int(u64::from(opt.dll_characteristics)),
        ),
        field(
        ...
        field(
            ...
            Value::Int(u64::from(opt.number_of_rva_and_sizes)),
        ),
    ])
}
